...
def bringDynamicalLoadDispData(cutoff_names = ["fomData/1.5/coSimData/"], cutoff_incr = [699], remove_corners=False):
    ...
    for i in cutoff_names:
        ...
        flLoadData = np.load(i+"/disp_data.npy")[map, :]
        flLoadData = np.vstack((flLoadData, np.load(i+"/velocity_data.npy")[map, :]))
        if remove_corners:
            dispData = np.load(i+"/load_data.npy")[2:-2, :]
        else:
            dispData = np.load(i+"/load_data.npy")
